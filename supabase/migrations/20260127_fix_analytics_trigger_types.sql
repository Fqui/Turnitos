-- Fix analytics trigger type mismatch error (UUID vs TEXT)
-- This explicitly casts IDs to TEXT in joins to handle cases where 
-- bookings table might have TEXT columns while referenced tables have UUID.
...
BEGIN
    ...
    FROM businesses b
    LEFT JOIN categories cat ON cat.id::text = b.category_id::text
    LEFT JOIN services s ON s.id::text = NEW.service_id::text -- Explicit cast to handle UUID/TEXT mismatch
    LEFT JOIN courts c ON c.id::text = NEW.court_id::text    -- Explicit cast
    WHERE b.id::text = NEW.business_id::text                 -- Explicit cast
...
-- Force trigger function update
COMMENT ON FUNCTION sync_booking_to_analytics IS 'Fixed types: Enforces text casting for joins';
